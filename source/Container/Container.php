...
class Container
{
     ...
     * @param array $params
     */
    public function resolveInstantiable(string $id, array $params = [])
    {
     ...
     */
    public function resolveDependencyList(array $dependencies, ParameterBag $params): array
    {
        if(!EnviromentSeup::$status){
            EnviromentSeup::setEnviroment();
        }
        return array_map(
    ...
    public function getAfterFire(string $id)
    {
        return $this->extends[get_parent_class($id) ?? null];
    }

    public function resolveAfterFire(string $id, object $instance)
    {
